...
Partition::Partition(const Graph& g, const int& subgraphs, bool GramSchmidt)
{
    ...
    for (int i = 0; i < num_of_eigenvec; i++) {
        ...
        laplacian_eigen_mat_.push_back(getOneLapEigenVec(
            lanczos.lanczos_vecs, tri_eigen_vecs, vector_index));
    }
    ...
    for (int vertex = 0; vertex < g.size(); vertex++) {
        ...
        for (int row = 0; row < num_of_eigenvec; row++) {
            colour +=
                pow(2, row) *
                signMedian(laplacian_eigen_mat_[row][vertex], median_vec[row]);
        }
        g.setColour(g.globalIndex(vertex), colour);
    }
#endif
...
Vector Partition::getOneLapEigenVec(DenseMatrix& lanczos_vecs,
                                    DenseMatrix& tri_eigen_vecs,
                                    const int& vector_index)
{
    ...
    // lanczos vector - m * n, tri_eigen_vecs - m * m, Ritz - m * n, Ritz_vector
    // - n
    ...
    for (int col = 0; col < col_size; col++) {
        for (int row = 0; row < row_size; row++) {
            laplacian_vector[col] +=
                lanczos_vecs[row][col] * tri_eigen_vec[row];
        }
    }
